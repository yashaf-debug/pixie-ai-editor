...
const AdjustmentPanel: React.FC<AdjustmentPanelProps> = ({ onApplyAdjustment, isLoading, cost, onEnhancePrompt }) => {
  const { t } = useLanguage();
  ...
  const presets = [
    { name: t('adjustmentPanel.presetBlur'), prompt: 'Apply a realistic depth-of-field effect, making the background blurry while keeping the main subject in sharp focus.' },
    { name: t('adjustmentPanel.presetEnhance'), prompt: 'Slightly enhance the sharpness and details of the image without making it look unnatural.' },
    { name: t('adjustmentPanel.presetWarmer'), prompt: 'Adjust the color temperature to give the image warmer, golden-hour style lighting.' },
    { name: t('adjustmentPanel.presetStudio'), prompt: 'Add dramatic, professional studio lighting to the main subject.' },
  ];
  ...
  const handleApply = () => {
    if (activePrompt) {
      ...
      onApplyAdjustment(activePrompt, ar, model);
    }
  };
  ...
  return (
    <div className="w-full flex flex-col gap-4 animate-fade-in">
      <h3 className="text-sm font-bold text-gray-800 dark:text-white">{t('adjustmentPanel.title')}</h3>
      ...
      <div className="relative w-full">
        ...
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full text-purple-600 hover:bg-purple-100 disabled:text-gray-400 disabled:cursor-not-allowed disabled:hover:bg-transparent dark:text-purple-400 dark:hover:bg-purple-900"
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
            <label className="block text-xs font-medium text-gray-600 mb-1 dark:text-gray-400">{t('generator.aspectRatio')}</label>
        ...
        <div>
            <label className="block text-xs font-medium text-gray-600 mb-1 dark:text-gray-400">Model</label>
      ...
      {activePrompt && (
        <div className="animate-fade-in flex flex-col gap-4 pt-2">
            ...
            >
                {t('adjustmentPanel.applyButton')} ({t('credits.costLabel', { cost: String(cost) })})
            </button>
        </div>
      )}
    </div>
